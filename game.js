...
function resizeCanvas() {
    const aspectRatio = 16 / 9; // Ширина / Высота scrn.width = window.innerWidth * 0.9; // 90% от ширины окна
    scrn.height = scrn.width / aspectRatio; // Высота по соотношению сторон

    // Если высота больше, чем 90% от высоты окна, устанавливаем по высоте if (scrn.height > window.innerHeight * 0.9) {
        scrn.height = window.innerHeight * 0.9;
        scrn.width = scrn.height * aspectRatio;
    }
    ...
    draw: function () {
        switch (state.curr) {
            case state.getReady:
                ...
                break;
            case state.gameOver:
                this.y = (scrn.height - this.gameOver.sprite.height) / 2.7;
